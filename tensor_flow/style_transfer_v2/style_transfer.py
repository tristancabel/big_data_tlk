...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        self.content_w = tf.constant(0.001)
        self.style_w = tf.constant(0.0000002)
    ...
    def _content_loss(self):
        ...
        '''
        P = self.content_img_layer_repr
        F = getattr(self.vgg, self.content_layer)
        
        coeff = tf.constant(0.25) / tf.reduce_prod(tf.to_float(tf.shape(P)))
        return tf.reduce_sum((F-P)**2 ) * coeff
        ###############################
    ...
    def _gram_matrix(self, F, N, M):
        ...
        """
        f_reshape = tf.reshape(F, (M, N))
        return tf.matmul(tf.matrix_transpose(f_reshape), f_reshape)
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        """
        dim_n = a.shape[3]
        dim_m = a.shape[1]*a.shape[2]
        a = self._gram_matrix(a, dim_n, dim_m)
        g = self._gram_matrix(g, dim_n, dim_m)

        coeff = 1 / (2. * dim_n * dim_m)**2
        return coeff*tf.reduce_sum(tf.squared_difference(g, a))
        ###############################
    ...
    def _style_loss(self):
        ...
        A = self.style_img_layers_repr
        
        for i in range(n_layers):
            layer_losses.append(self.style_layer_w[i] *
                                self._single_style_loss(A[i], getattr(self.vgg, self.style_layers[i])))

            
        return reduce(lambda a,b: a+b, layer_losses)
        ###############################
    ...
    def losses(self):
        with tf.variable_scope('losses') as scope:
            with tf.Session() as sess:
                ...
                self.content_img_layer_repr = sess.run(getattr(self.vgg, self.content_layer))

            with tf.Session() as sess:
                # assign style image and get style layers repr
                sess.run(self.input_img.assign(self.style_img))
                self.style_img_layers_repr = sess.run([getattr(self.vgg, layer) for layer in self.style_layers])

            self.content_loss = self._content_loss()
            self.style_loss = self._style_loss()
            self.total_loss = tf.add(tf.multiply(self.content_w, self.content_loss) ,
                                     tf.multiply(self.style_loss, self.style_loss))
